...
private.getFileMd5 = function(file, cb){
  var crypto = require('crypto');
  var fs = require('fs');
...
private.attachApi = function () {
  ...
  router.post('/', function (req, res, next) {
    ...
    req.sanitize(req.body, {
      ...
      if (!report.isValid) {
                ...
                return res.json({ success: false, error: report.issues, body:req.body });
      }
      console.log("============================= ok with da create ============ ");
      if (req.request.files && req.request.files.length > 0 ){
        console.log("Will download file to local directory");
      }
  ...
  router.post('/register', function (req, res, next) {
    ...
    req.sanitize(req.body, {
      ...
      function fileReadyCallback(err){
        if (err){
          return res.json({ success: false, error: err});
        }
        library.balancesSequence.add(function (cb) {
      ...
      if (body.link) {
        ...
        private.getFileMd5(files[0].path, function(err, md5){
          if (err){
  ...
  router.post('/launch', function (req, res, next) {
    if (library.config.dapp.masterpassword && req.body.master !== library.config.dapp.masterpassword) {
      return res.json({ success: false, error: "Invalid master password:"+ req.body.master + ", " + library.config.dapp.masterpassword });
    }
...
private.download (destPath, link, serialCb) {
